...
export default function FeatureNode({ id, data }: NodeProps<FeatureNodeData>) {
  ...
  // ป้องกันกรณีที่ data.category ไม่มีค่า (Fallback เป็น Scripting)
  ...
  return (
    <div className={`w-72 bg-slate-900/80 backdrop-blur-md border ${style.border} rounded-xl ${style.glow} transition-all duration-200 hover:border-slate-400/50 relative`}>
      ...
      <Handle 
        ...
        className={`!w-2.5 !h-8 ${style.handle} !border-2 !border-slate-950 !rounded-md !-left-3 cursor-crosshair transition-transform hover:scale-125 z-20`} 
      />
      ...
      <div className="rounded-xl overflow-hidden">
        ...
        <div className="w-full bg-slate-950/60 h-1">
          <div 
            className={`bg-gradient-to-r ${style.bar} h-1 transition-all duration-500 ease-out`}
            ...
            {(data.tasks || []).map((task) => (
              ...
              >
                ...
                <span className={`transition-all duration-200 leading-relaxed ${
                  task.done ? 'line-through text-slate-500 opacity-60' : 'text-slate-300 group-hover:text-slate-100'
                }`}>
                  {task.text}
                </span>
              </label>
      ...
      <Handle 
        ...
        className={`!w-2.5 !h-8 ${style.handle} !border-2 !border-slate-950 !rounded-md !-right-3 cursor-crosshair transition-transform hover:scale-125 z-20`} 
      />
